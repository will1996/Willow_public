...
class Subject{
        typedef void(*invoker_t)(Observer *, Message*);
    struct Invocation {
        invoker_t inv;
        Observer* obs;
    ...
    public :
        template<typename SUBTYPE, typename Obs, void(Obs::*Method)(Message*)>//register method for objects
        //TODO enforce Observer interface with concept 
        void permit(Obs* obs){
                //get the observer's unique ID to track it
               auto uniqueID = obs->getID();
        ...
        void notify(SUBTYPE* t){
                //get the list of uniqueID's for the trigger filter
            for (wilo_ID_type id : m_triggerFilter[typeid(SUBTYPE)]){
                m_registry[id][typeid(SUBTYPE)].invoke(t);//call the bound method for each ID
            };
        };

        void revoke(wilo_ID_type id){
                    m_registry.erase(id);//remove all bound methods for the ID 
        };
    private:
            template<typename SUBTYPE, typename Obs, void(Obs::*Method)(Message* subtype)>
        static void invoker( Observer* obs, Message* msg){
                (static_cast<Obs*>(obs)->*Method)(static_cast<SUBTYPE*>(msg));
        }
        std::map<wilo_ID_type, std::map<std::type_index,Invocation> >  m_registry;
        std::map<std::type_index,std::vector<wilo_ID_type> > m_triggerFilter;
};
