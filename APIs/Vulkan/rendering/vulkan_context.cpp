...
namespace wilo {
    VulkanRenderingContext::VulkanRenderingContext(wilo::SharedPointer<Window> window )
        : wilo::RenderingContext(window),
        m_window(window),
    ...
    void VulkanRenderingContext::initialize(RenderingContext::Info info){

        m_enableValidationLayers = info.enableGraphicsDebugging;
    ...
    VkInstance VulkanRenderingContext::getInstance()
    {
        return m_instance;
     }

    VkPhysicalDevice VulkanRenderingContext::getPhysicalDevice()
    {
        return m_physicalDevice;
    }

    VkDevice VulkanRenderingContext::getDevice()
    {
        return m_device;
    }

    VkSurfaceKHR VulkanRenderingContext::getSurface()
    {
        return m_surface;
    }
    std::string VulkanRenderingContext::getShadersPath(){
    ...
    std::vector<const char*> VulkanRenderingContext::getInstanceExtensions(RenderingContext::Info info) {
    ...
    std::vector<const char*> VulkanRenderingContext::getInstanceLayers(RenderingContext::Info info)
    {
    ...
    void VulkanRenderingContext::createInstance(RenderingContext::Info info) {
        VkApplicationInfo appInfo = {};
        appInfo.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
        appInfo.engineVersion = WILO_ENGINE_VERSION;
        appInfo.pEngineName = "Willow";
        ...
        std::vector<const char*> instanceExtensions = getInstanceExtensions(info);

        instanceInfo.ppEnabledExtensionNames = instanceExtensions.data();
        instanceInfo.enabledExtensionCount = instanceExtensions.size();


        std::vector<const char*> instanceLayers = getInstanceLayers(info);
    ...
    void VulkanRenderingContext::createSurface() {
        if (m_window->getInfo().API != wilo::WindowingAPICode::GLFW) {
            throw std::runtime_error("Vulkan currently only supports GLFW");
        }

        GLFWwindow* glfwWindow = (GLFWwindow*)(m_window->getNativeWindow());
    ...
    wilo::vk::SwapChainSupportDetails querySwapChainSupport(VkPhysicalDevice device, VkSurfaceKHR surface) {
        wilo::vk::SwapChainSupportDetails details;
    ...
    //public facing version of querySwapChainSupportDetails;
      vk::SwapChainSupportDetails VulkanRenderingContext::getSwapChainSupportDetails(){
          return querySwapChainSupport(m_physicalDevice,m_surface);
      }

    std::vector<const char*> VulkanRenderingContext::getDeviceExtensions(RenderingContext::Info info)
    {
...
bool VulkanRenderingContext::physicalDeviceIsSuitable(VkPhysicalDevice device, VulkanRenderingContext::Info info){
    vk::QueueFamilyIndices queueFamilies =  vk::findQueueFamilies(device, m_surface);
    bool swapChainSupportAdequate = false;
    bool extensionsSupported = checkDeviceExtensionSupport(device,getDeviceExtensions(info));
...
void VulkanRenderingContext::selectPhysicalDevice(VulkanRenderingContext::Info info){
    ...
    for(VkPhysicalDevice device : devices){
        if(physicalDeviceIsSuitable(device,info)){
...
*/
void VulkanRenderingContext::createLogicalDevice(RenderingContext::Info info ) {
    ...
    std::vector<const char* > deviceExtensions = getDeviceExtensions(info);
