...
namespace wilo{
	...
	VulkanSwapchain::VulkanSwapchain(wilo::SharedPointer<VulkanRenderingContext> pcontext) :
		wilo::Swapchain(pcontext),
		p_context(pcontext),
		m_supportDetails(p_context->getSwapChainSupportDetails())
	{																 }
		

	void VulkanSwapchain::initialize(wilo::Swapchain::Info info){
		m_info =info;
		createVkSwapchain();
		getSwapchainImages();
	...
	void VulkanSwapchain::reinitialize(wilo::Swapchain::Info info){
		m_supportDetails = p_context->getSwapChainSupportDetails();
		reclaim();
		initialize(info);
	}
	...
	void 		   VulkanSwapchain::chooseSwapChainExtent(){
			...
			VkExtent2D extent = {m_info.frameWidthPix,m_info.frameHeightPix};
			
			extent.width = std::max(m_supportDetails.capabilities.minImageExtent.width,
														m_info.frameWidthPix);
			extent.height = std::max(m_supportDetails.capabilities.minImageExtent.height,
														m_info.frameHeightPix);
