...
bool Device::f_physicalDeviceSuitable(VkPhysicalDevice device, VkSurfaceKHR surface) {
  ...
  VkQueue Device::getGraphicsQueue(){
    if(m_queues.empty())
      throw std::runtime_error("wilo::vk::device must be initialized before its graphics queue is used");
    return m_queues[m_graphicsQueueIndex];
  }

  VkQueue Device::getPresentQueue(){
    if(m_queues.empty())
      throw std::runtime_error("wilo::vk::device must be initialized before its present queue is used");
